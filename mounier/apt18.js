...
jQuery(function ($) {
  ...
  //  var machine = [6775, 10217, 13583, 16967, 20375, 23777, 27113, 30466, 33842, 37209, 40651, 43992, 47371, 50659, 55091, 57497, 60840, 64245, 67580, 71863, 74326, 77769, 81233, 84448, 87838, 91228, 94558, 98394];
    ...
    var transitions = [
      ...
      //     }
      //     //console.log('xNO: '+i+' '+p.toX+' yNO: '+i+' '+p.toY);
      //   }
      // },
      ...
      function() {
        ...
        var startParticle = 200;
        var sqrStartParticle = startParticle + 40;

        var objLength = pixels.length
        var totalLeft = 40
        var totalSize = objLength - totalLeft;

        var sqrSides = Math.floor(totalLeft/4);
        var sqrSideOne = totalSize + sqrSides;
        var sqrSideTwo = totalSize + sqrSides*2;
        var sqrSideThree = totalSize + sqrSides*3;
        var sqrSideFour = totalSize + sqrSides*4;

        var sideOne = Math.floor(totalSize*0.2);
        var sideTwo = (totalSize*0.3) + sideOne;
        var sideThree = (totalSize*0.2) + sideTwo;
        var sideFour = (totalSize*0.3) + sideThree;
        
        square(sideOne, sideTwo, sideThree, sideFour, startParticle, 7, 120, totalSize, 0);
        square(sqrSideOne, sqrSideTwo, sqrSideThree, sqrSideFour, sqrStartParticle, 7, sqrStartParticle, objLength, totalSize);
        function square(sideOne, sideTwo, sideThree, sideFour, startParticle, size, particleY, loopLength, loopStart){

          var lastParticle = startParticle;
          
          for(i = loopStart; i<loopLength; i++ ) {
            var p = pixels[i];
            if(p.flightMode != 2) {
              impulsX = 0;
              impulsY = 0;
              ...
              if(i<sideOne){
                p.toY = particleY;
                p.toX = lastParticle+p.toSize;
                lastParticle = p.toX;

                if(i == (sideOne-1)){
                  var nextParticleX = p.toX;
                  var nextParticleY = p.toY;
                }
              }
              if(i>=sideOne && i<sideTwo){
                if(i == sideOne){
                  lastParticle = nextParticleY;
                }

                p.toX = nextParticleX;
                p.toY = lastParticle+p.toSize;
                lastParticle = p.toY;

                if(i == (sideTwo-1)){
                  var nextParticle2X = p.toX;
                  var nextParticle2Y = p.toY;
                }
              }
              if(i>=sideTwo && i<sideThree){
                if(i == sideTwo){
                  lastParticle = nextParticle2X;
                }

                p.toY = nextParticle2Y;
                p.toX = lastParticle-p.toSize;
                lastParticle = p.toX;

                if(i == (sideThree-1)){
                  var nextParticle3X = p.toX;
                  var nextParticle3Y = p.toY;
                }
              }
              if(i>=sideThree && i<sideFour){
                if(i == sideThree){
                  lastParticle = nextParticle3Y;
                }

                p.toX = nextParticle3X;
                p.toY = lastParticle-p.toSize;
                lastParticle = p.toY;
              }
            }            
  ...
  //   }
    ...
    if(canPlayType.match(/maybe|probably/i)) {
      $('#audio').attr('src', 'http://flateric.columba.uberspace.de/canvas/thankyou.ogg');
    } else {
      $('#audio').attr('src', 'http://flateric.columba.uberspace.de/canvas/thankyou.mp3');
    }
