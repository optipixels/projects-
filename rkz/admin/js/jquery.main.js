$(document).ready(function(){
	...
	$('.news-form').on('submit', function(e){
		...
		if(formId == 'news-edit'){
			var id = $.trim($('.id').val());
			var post_url = 'php/news-edit.php';
			var success_msg = 'News has been Edited Successfully';
		}else{
			var post_url = 'php/news-add.php';
			var success_msg = 'News has been Added Successfully';
		}
	...
	$('.sub-cat-form').on('submit', function(e){
		e.preventDefault();
		var formId = $(this).attr('id');
		
		if(formId == 'sub-cat-edit'){
			var id = $.trim($('.id').val());
			var post_url = 'php/sub-cat-edit.php';
			var success_msg = 'Sub Category has been Edited Successfully';
		}else{
			var post_url = 'php/sub-cat-add.php';
			var success_msg = 'Sub Category has been Added Successfully';
		}

		var title = $.trim($('.sub-cat-title').val());
		var prod_cat = $.trim($('.sub-cat-cat').val());
		var result = '';
	...
	$('.prod-form').on('submit', function(e){
        ...
        if(check_sub_cat != 0){
            if(prod_sub_cat == 'select'){
                result += "<p>Please Select Sub Category.</p>";
            }
		}
	
		if(result == ''){
			$('.prod-form>.message-area').fadeIn(500).removeClass('error').removeClass('success').addClass('loading').html('Authenticating, Please Wait!!');
			$.ajax({
				url: post_url,   	// Url to which the request is send
				type: "POST",					// Type of request to be send, called as method
				data:  new FormData(this), 		// Data sent to server, a set of key/value pairs representing form fields and values 
				...
				success: function(data){		// A function to be called if request succeeds
					if(data == "0"){
						$('.prod-form>.message-area').fadeIn(500).removeClass('error').removeClass('loading').addClass('success').html(success_msg);
						$('.prod-form>.message-area').delay(5000).fadeOut(500);
					}else{
						$('.prod-form>.message-area').fadeIn(500).removeClass('loading').removeClass('success').addClass('error').html(data);
						$('.prod-form>.message-area').delay(5000).fadeOut(500);
					}
				}
			});
		}else{
			$('.prod-form>.message-area').fadeIn(500).removeClass('loading').removeClass('success').addClass('error').html(result);
			$('.prod-form>.message-area').delay(5000).fadeOut(500);
		}
	
	});
    
    //Products Sub Category Select
    $('.prod-cat').on('change', function(){
        var selected = $.trim($(this).val());
        
        $('.select-sub-cat').find('.message-area').fadeIn(500).removeClass('error').addClass('loading').html('Searching Sub Categories');
        $.post('php/cat-change.php', {cat_id: selected}, function(data){
            if(data=='0'){
                $('.select-sub-cat').find('select').remove();
                $('.select-sub-cat').find('.message-area').fadeIn(500).removeClass('loading').removeClass('success').addClass('error').html('No Sub Categories Found');
				$('.select-sub-cat').find('.message-area').delay(5000).fadeOut(500);
            }else{
                $('.select-sub-cat').find('select').remove();
                $('.select-sub-cat').append(data);
                $('.select-sub-cat').find('.message-area').fadeOut(500);
            }
        });
    });
