...
if(!empty(mysql_real_escape_string($_POST['prod-sub-cat']))){
    $sub_cat_id =  mysql_real_escape_string($_POST['prod-sub-cat']);
}else{
    $sub_cat_id = '0';
}
...
if($uploadImage['status'] == 'false'){
	...
	$query = mysql_query("UPDATE prod SET title='$title', article='$article', featured='$feat', description='$prod_desc', url='$url_img', cat_id='$cat_id', sub_cat_id='$sub_cat_id' WHERE id = $id_edit");
	if($query){
		echo '0';
	}else{
